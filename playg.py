...
def main():
    ...
    group_folder = os.path.join(
        rp_dataset_folder, f'group_{group}', 'top_surfaces')
    output_folder = os.path.join(
        rp_dataset_folder, f'group_{group}', 'rendered_o3d')
    if not os.path.exists(group_folder):
        ...
        for frag in frags:
            ...
            pdb.set_trace()
            edges = torch.cat([torch.tensor(f[:, :2]), torch.tensor(
                f[:, 1:]), torch.tensor(f[:, 0:3:2])], dim=0)
