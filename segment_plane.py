...
def main():
    ...
    for frag in frags:
        ...
        mesh_ply = open3d.io.read_triangle_mesh(os.path.join(group_folder,'processed',f'{frag[:-4]}.obj'), enable_post_processing=True)

        plane_model, inliers = pcd.segment_plane(distance_threshold=1,
                                                 ransac_n=3,
                                                 num_iterations=1000)
        top_surface = pcd.select_by_index(inliers)
        mesh_ply_top_surf = mesh_ply.select_by_index(inliers)#, cleanup=False)
        #o3d.visualization.draw_geometries([mesh_ply_top_surf])
